...
# Cf. README.MD for brief explanations how to use these functions in a main script.
#
# ----------------------------------------------------------------------------------------------------------------------------
import numpy as np
import h5py
...
# ---------------------------------------------------------------------------------
# model for 2 coupled diffusive modes (switching diffusion) according to Roosen-Runge, Bicout, and Barrat, J.Chem.Phys. 144, 204109 (2016):
# see also Grimaldo et al., Phys.Chem.Chem.Phys. 17, 4645 (2015)
...
# ---------------------------------------------------------------------------------
def array2vector( x ): # transform a numpy array to a vector for use by scipy.opimize.curvefit
	try:
		len(x[0])
	except:
		out = x
...
# ---------------------------------------------------------------------------------
def vector2array( x, q ): # transform a vector back to a numpy array
	try:
		len(q)
	except:
		out = x
...
# ---------------------------------------------------------------------------------
#
# build the model S(q,omega) by parsing keywords - note that some keyword arguments are not used here but passed on to "convoluted_model_parser":
...
#
	class out:      # object of returned values
		pass
	ID2O = np.zeros( np.shape( hw ) ) # initialize storage
	...
	for key, value in kwargs.items():
		...
		if key == 'BGFlat':
			bgf = value
	for key, value in kwargs.items():
		if key == 'SQWModel':               # select the model for the scattering function by the user-provided keyword
			# The pattern always follows this scheme: the parameters are stored either in a vector p[:] in case of a fit at a single q-value
			# or in a matrix p[k,:], where k is the q-index, in case of a "global" fit for all q-values at once. 
			# The order of the parameters in p is
			# - pairs of ( Lorenztian intensity, Lorentzian width ) for the given number of Lorentzians
			# - elastic intensity (optional)
			# - background slope (optional)
			# - background offset (optional)
			# Both variable fit parameters fp[:] controled by curve_fit, as well as fixed parameters such as solvent linewiths can be passed.
			# The model block of code calculates p from fp and the fixed parameters depending on the specific model.
			if value == 'IN5SolventPerQ':
			...
			elif value == 'BATSSolventFixedGammaPerQ':
				k      = qn[ 0 ]
				p      = np.zeros( 5 )
				p[ 0 ] = fp[ 0 ]    # intensity of first solvent Lorentzian
				...
				ng = 1                                             # number of global parameters: D
				...
				for k in range( 0, len( qn ) ):
					...
					p[ k, 9 ]  = bgs[ k ]                                  # background slope
					p[ k, 10 ] = bgf[ k ]                                  # background offset
			elif value == 'IN5Guest1LorGlobalSolventAmpFixed':
				k = qn[0]                        # fit parameters: Lorentzian amplitudes, width of guest molecule Lorentzian, amplitude of Dirac
				p = np.zeros( [ len(qn), 11 ] )  # parameter matrix to be returned	
				for k in range( 0, len( qn ) ):
					p[ k, 0 ]  = fp[ 0 ] * fp[ 3+k ]             # intensity of the guest molecule Lorentzian
					p[ k, 1 ]  = fp[ 1 ] * q[ k ]**2 / ( 1 + fp[ 1 ] * q[ k ]**2 * fp[ 2 ] )                # width guest molecule Lorentzian
					p[ k, 2 ]  = fp[ 3+2*k ]                     # intensity of the guest molecule Lorentzian
					p[ k, 3 ]  = fp[ 3+3*k ]
					p[ k, 4 ]  = ID2O[ k, 0 ]                    # intensity 1st solvent Lorentzian
					p[ k, 5 ]  = GD2O[ k, 0 ]                    # width 1st solvent Lorentzian
					p[ k, 6 ]  = ID2O[ k, 1 ]                    # intensity 2nd solvent Lorentzian
					p[ k, 7 ]  = GD2O[ k, 1 ]                    # width 2nd solvent Lorentzian
					p[ k, 8 ]  = fp[ 3+4*k ]                     # amplitude of guest molecule Dirac
					p[ k, 9 ]  = bgs[ k ]                        # background slope
					p[ k, 10 ] = bgf[ k ]                        # background offset
...
def sumNVoigts( x, l, r, p ):
	...
	# attention is necessary to the correct shape of the resolution maxtrix
...
def convoluted_model_parser( hwq, q, qn, pq, r, **kwargs ): 
	...
	# This function also creates information for later plotting fit components.
	#
	class out:
		pass
	#
...
# ---------------------------------------------------------------------------------
def loadIN16B( filename ):   # read Mantid-reduced IN16B QENS data, assuming data are in 2Theta
	class out:
		pass
...
# ---------------------------------------------------------------------------------
def loadIN16B_Q( filename ):   # read Mantid-reduced IN16B QENS data, already converted to Q
	class out:
		pass
	f = h5py.File( filename, 'r' )
	out.x    = np.transpose( np.array( f['/mantid_workspace_1/workspace/axis1/']  ) )
...
# ---------------------------------------------------------------------------------
def loadIN5( filename, **kwargs ):   # read lamp-reduced IN5 QENS data, optionally lineup given q-indices
	class out:
		pass
	f = h5py.File( filename, 'r' )
	...
	for key, value in kwargs.items():
		if key == 'lineup':      # optional lineup of peaks to zero energy channel
			idx = value
			for k in idx:
				x   =   out.hw
				y   =  out.sqw[ :, k ].copy()
				dy  = out.dsqw[ :, k ].copy()
				s   = np.argmin( np.abs( x ) ) - np.argmax( y ) # shift by this number of channels
				ys  = np.roll(  y, s )
				dys = np.roll( dy, s )
				if s < 0:
					ys[ s: ]  = 0       # delete channels that are "rolled over"
					dys[ s: ] = np.inf
				else:
					ys[ :s ]  = 0       # delete channels that are "rolled over"
					dys[ :s ] = np.inf
				print( 'Lining up by ', s, 'channels' )
				out.sqw[ :, k ]  =  ys
				out.dsqw[ :, k ] = dys
	return( out )
...
# resolution described by a single Gaussian or a sum of Gaussians ( number determined from length of sig)
	import sys
...
# ----------------------------------------------------------------------------------------------------------------------------
def resolution_function( q, hw, sqw, dsqw, *args, **kwargs ):  # fit spectrometer resolution, function for several instruments
# The function aims to fit a sum of several Gaussian functions to the measured resolution spectrum.
# The Gaussians are allowed to be shifted relative to zero energy transfer.
# The use of this function is challenging, as initial guesses and bounds may need to be adjusted to the specific experiment
# taking into account for instance the sample geometry, and the wavelength used.
# The length of the vector of start values defines the number of Gaussians for the resolution function.
# Lowest entry in this vector: constant background, then subsequently: (a_i, sigma_i, x0_i)
	...
	datapath = ''
	filenameextension = ''
	cryofit  = None # for fitting a cryostat background
	rawvanad = None
	verbose  = 0
...
#			print( 'x_min', x.min(), 'x_max', x.max()  )
			...
			if use10A: # single Gaussian
				f01 = [ 0,     8e1,        10,     0 ] # start values for single Gauss
				l01 = [ 0,      0,          3,  -100 ] # bounds for single Gauss
				u01 = [ np.inf, np.inf, np.inf,  100 ]
				popt, pcov = curve_fit( lambda x, *p: wrapper_resolution_func( x, len( f01 ), p ), hwd, y, p0=f01, sigma=dy, bounds=(l01,u01), ftol=1e-15, xtol=1e-15 )
			else:	
...
#			try:
			if cryofit is not None:
				plt.ylim( [ 0.005 * np.array( [ resolution_model_IN5( np.linspace( -erange, erange, 100 ), popt[0], popt[1::3], popt[2::3], popt[3::3] ).min(), 1e-9 ] ).max(),
				...
				plt.ylim( [ 0.001 * np.array( [ resolution_model_IN5( np.linspace( -20, 20, 100 ), popt[0], popt[1::3], popt[2::3], popt[3::3] ).min(), 1e-9 ] ).max(),
